...
## ℹ️ 서비스 설명
## 주요 기능
- 노트북을 사용하는 동안 바른 자세를 유지할 수 있도록 돕는 프로그램입니다.
- 'mediapipe' 라이브러리를 이용하여 웹캠만으로 사용자의 자세 정보를 정확하게 추출합니다.
- 사용자에 따른 바른 자세 정보를 추출하여 데이터베이스에 저장합니다.
- '거북목, 화면과 가까움, 틀어짐'의 세 가지 자세를 판단합니다.
- 바르지 못한 자세를 취할 경우 경고 메시지와 경고음을 통해 알려줍니다.
- PiP 기능을 통해 실시간으로 자신의 자세를 나타낸 점을 웹캠 화면에서 확인할 수 있습니다.
- 설정에서 웹캠 화면의 모습과 기록되는 자세를 자유롭게 변경할 수 있으며, 해당 정보를 데이터베이스에 저장합니다.
- 캐릭터 모드에서는 사용자의 자세에 따라 화면에 그림이 나타나며, 거북목 자세 시 머리 부분이 거북이 얼굴로 바뀝니다.
...
### 4.2 기술 설명
- mediapipe를 이용하여 실시간으로 자세를 추적하고, 랜드마크 좌표를 저장합니다.
<br/>
...
### 5.1 주요 기능
- Chrome Extension을 이용하여 랜드마크를 나타낸 점을 PiP로 띄웁니다.
...
- 경고음이 울릴 때, PiP 화면 점의 색과 크기가 바뀌며 잘못된 자세임을 알립니다.

### 5.2 기술 설명
- canvas를 video stream으로 전환해 실시간으로 랜드마크를 PiP 화면에 띄웁니다.
...
- landmark의 z값을 바른 자세의 값과 비교하여 일정 기준 이하로 내려갈 경우 화면과의 거리가 너무 가깝다고 판단합니다.다.
